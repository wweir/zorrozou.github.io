...
综合来看，对于linear和striped这种简单target，DM框架的开销主要是bio克隆的slab分配，大约在**数十纳秒**量级。在SATA HDD（毫秒级延迟）和普通SSD（百微秒级延迟）场景下完全可以忽略。但在使用Intel Optane这类超低延迟设备（微秒级）且IOPS达到百万级的极端场景下，DM框架的开销可能占到总延迟的1%~5%。
...
单次随机读 4K：
  ...
  T_map ≈ 20ns，T_disk ≈ 数百μs（SATA HDD）或数十μs（NVMe SSD）
...
- 内存：121GB
- 磁盘：12块10.9TB SATA本地盘（virtio-blk）
...
需要注意的是，本文的测试是在SATA本地盘（virtio-blk）环境下进行的，单盘IO延迟本身就在毫秒级。对于使用NVMe SSD或Intel Optane这类超低延迟设备的场景，单次IO延迟可以低至微秒级，此时CPU路径上的开销占比会更大一些。如果你的环境是这类设备且IOPS非常高（百万级），建议自行验证。但即便如此，2的幂的建议仍然是合理的工程实践——它不会带来坏处，而且内核代码确实为此做了优化。
...
- 腾讯云CVM，4块SATA本地盘通过`dmsetup`创建stripe设备（4条带，chunk=128K）
...
> **建议5：理解RMW/RCW的代价模型是优化RAID5写性能的基础。rmw_level的最优值取决于IO模式，而非简单的RAID级别。**
>
> 对于RAID5和RAID6：默认保持`rmw_level=1`，让内核动态选择RMW/RCW。如果工作负载以4K顺序写为主且队列深度较浅（如同步日志写入），可以尝试`rmw_level=0`（禁用RMW），在RAID5上可带来最高71%的提升（详见下方实验验证）。避免使用`rmw_level=2`——在RAID6顺序写场景下可能造成50%的性能下降。
>
> 无论哪种策略，最有效的优化始终是**让写操作对齐到stripe大小，直接触发FSW**——彻底绕过RMW/RCW。

#### 实验验证：rmw_level对RAID5/RAID6写性能的实际影响

上面的分析给出了`rmw_level`的理论建议，但理论分析有一个盲区——它只考虑了**无缓存时**的读IO次数，没有考虑stripe cache的影响。而3.3.4节已经说明了"缓存命中可以改变最优策略"。那么在实际IO场景中，`rmw_level`的不同取值到底能带来多大的性能差异？哪些场景下差异显著，哪些场景下可以忽略？我们需要用实验数据来回答。

**测试环境**：腾讯云CVM，AMD EPYC 7K83 32核，121GB内存。RAID5：5×10.9TB SATA HDD（virtio-blk），chunk=256K，left-symmetric。RAID6：6×10.9TB SATA HDD，chunk=256K。fio 3.19，Direct IO，libaio引擎，runtime=20s × 3轮取平均。

测试覆盖8种写负载，从"最差场景"到"最优场景"的完整频谱：

| 负载 | 描述 | 设计意图 |
|------|------|---------|
| W1 | randwrite 4k d=1 | 逐个提交随机写，几乎100%触发RMW/RCW |
| W2 | randwrite 4k d=32 | 中等并发随机写 |
| W3 | randwrite 4k d=128 j=4 | 高并发随机写 |
| W4 | seqwrite 4k d=1 | 顺序写低队列，plug效果有限 |
| W5 | seqwrite 4k d=32 | 顺序写中队列，plug开始生效 |
| W6 | seqwrite 4k d=128 | 顺序写深队列，充分触发FSW |
| W7 | randwrite 256k d=32 | 大块随机写 |
| W8 | seqwrite 1M d=16 | 大块顺序写，对照基线 |

**RAID5 测试结果（IOPS，3轮平均）**

| 负载 | 描述 | rmw=0 | rmw=1（默认） | rmw=2 | 0 vs 1 | 2 vs 1 |
|------|------|------:|------:|------:|-------:|-------:|
| W1 | randwrite 4k d=1 | 18 | 22 | 22 | -18% | 0% |
| W2 | randwrite 4k d=32 | 55 | 65 | 67 | -15% | +3% |
| W3 | randwrite 4k d=128 j=4 | 89 | 99 | 97 | -10% | -2% |
| W4 | seqwrite 4k d=1 | 2,579 | 1,510 | 1,171 | **+71%** | **-22%** |
| W5 | seqwrite 4k d=32 | 12,964 | 9,165 | 9,423 | **+41%** | +3% |
| W6 | seqwrite 4k d=128 | 21,484 | 18,119 | 18,634 | **+19%** | +3% |
| W7 | randwrite 256k d=32 | 35 | 45 | 50 | -22% | +11% |
| W8 | seqwrite 1M d=16 | 793 | 944 | 951 | -16% | +1% |

**RAID6 测试结果（IOPS，3轮平均）**

| 负载 | 描述 | rmw=0 | rmw=1（默认） | rmw=2 | 0 vs 1 | 2 vs 1 |
|------|------|------:|------:|------:|-------:|-------:|
| W1 | randwrite 4k d=1 | 18 | 17 | 22 | +6% | +29% |
| W2 | randwrite 4k d=32 | 54 | 54 | 55 | 0% | +2% |
| W3 | randwrite 4k d=128 j=4 | 89 | 86 | 84 | +3% | -2% |
| W4 | seqwrite 4k d=1 | 2,210 | 2,238 | 1,109 | -1% | **-50%** |
| W5 | seqwrite 4k d=32 | 13,264 | 10,881 | 9,922 | **+22%** | -9% |
| W6 | seqwrite 4k d=128 | 16,766 | 17,974 | 18,347 | -7% | +2% |
| W7 | randwrite 256k d=32 | 35 | 42 | 45 | -17% | +7% |
| W8 | seqwrite 1M d=16 | 787 | 939 | 941 | -16% | 0% |

这两组数据中有几个值得深入分析的现象：

**1. 4K随机写（W1~W3）：rmw_level影响很小，方向符合理论预期。**

rmw=0（强制RCW）比rmw=1慢10%~18%，而rmw=1和rmw=2几乎无差。这与无缓存代价模型一致——5盘RAID5修改1个chunk时，RMW读2次（旧数据+旧P），RCW读3次（其余3个数据盘），RMW少读1次。但差异的**绝对值很小**（18 vs 22 IOPS），因为瓶颈在SATA盘的毫秒级寻道延迟，而非RMW/RCW的策略差异。RAID6的随机写结果也类似——三种配置的IOPS在17~22之间波动，差异在测量噪声范围内。

**2. 4K顺序写低队列（W4~W5）：rmw=0显著更优，这与简单理论模型矛盾。**

RAID5 W4（seqwrite 4k d=1）中，rmw=0比rmw=1快71%（2,579 vs 1,510 IOPS）——这是整组实验中最大的差距。按简单模型，4K顺序写也是k=1（每次修改1个chunk），应该和随机写一样RMW更优才对。**为什么顺序写下RCW反而大幅领先？**

答案就在3.3.4节讨论的**stripe cache效应**。顺序写与随机写有本质区别——连续的4K块命中同一stripe的相邻chunk。考虑一个具体的例子：

```
【5盘RAID5顺序写D0→D1→D2→D3，stripe cache的影响】

  写D0时：cache中无数据
    RMW读：旧D0 + 旧P = 2次
    RCW读：D1 + D2 + D3 = 3次  →  RMW更优

  写D1时：D0已在cache中
    RMW读：旧D1 + 旧P = 2次（D0的cache对RMW没用）
    RCW读：D2 + D3 = 2次（D0已在cache，不用读！）
    →  两者代价相同

  写D2时：D0、D1都在cache中
    RMW读：旧D2 + 旧P = 2次
    RCW读：D3 = 1次（D0、D1都在cache！）
    →  RCW反超RMW！

  写D3时：D0、D1、D2都在cache中
    RCW读：0次 → 实际上变成了FSW！
```

随着同一stripe中越来越多的块被顺序写入，cache中的已有数据不断累积，RCW的实际读IO次数持续下降——从3次降到2次、1次、最终0次。而RMW始终需要读旧数据+旧校验，无法从cache中获益。这就是rmw=0（强制RCW）在顺序写场景下大幅领先的根本原因。

**这个发现恰好验证了3.3.4节的核心论点——"不能简单比较修改块数，缓存命中可以改变最优策略"。** 我们在3.3.4节用一个例子说明了D0在cache中可以让RCW少读一次，但没有预见到这个效应在顺序写中被**系统性放大**——每写一个新块，cache中的数据就多一块，RCW的优势逐步积累，最终颠覆了RMW的理论优势。

**3. 高队列深度下（W6），rmw_level影响趋于消失。**

RAID5 W6（d=128）中三种配置的IOPS分别为21,484 / 18,119 / 18,634，差距收窄到±19%。这完全符合3.5节plug/unplug实验的结论——高iodepth下plug积攒足够多的连续IO触发FSW，FSW路径下所有数据盘都被覆盖，不需要读任何旧数据，直接计算校验写入，完全绕过了RMW/RCW选择。rmw_level只影响少量未能凑满整个stripe的"边角"写操作。

**4. 大块IO（W7/W8）：rmw=0反而更差。**

256K随机写和1M顺序写时，rmw=0比默认值低16%~22%。这也符合理论——大块IO覆盖chunk的大部分甚至整个stripe，RMW只需读少量旧数据做差分更新，而RCW需要读所有未修改块重算校验，代价反而更高。强制rmw=0剥夺了内核在这些场景下选择更优路径的能力。

**5. RAID6的rmw=2在顺序写d=1下暴跌50%。**

RAID6 W4中，rmw=2（偏好RMW）仅1,109 IOPS，比rmw=1的2,238慢了一半。这验证了RAID6的RMW代价确实更大——RMW需要同时读旧P和旧Q（读次数=k+2），且需要分别对P和Q做差分更新（两次XOR/GF运算+两次异步写）。但**这并不意味着应该强制rmw=0**——从数据来看，rmw=0和rmw=1在大多数场景下表现接近，而rmw=0在大块IO场景下有16%的劣势。内核默认的动态选择（rmw_level=1）已经能处理好RAID6的双校验代价：当RMW的代价确实更高时内核会自动选RCW，不需要人为强制。

**实验结论：**

1. **rmw_level的最优值取决于IO模式，而非RAID级别。** 不存在一个"对RAID5最好"或"对RAID6最好"的通用最优值。
2. **4K顺序写低队列场景下，rmw=0（禁用RMW）在RAID5上可带来最高71%的提升。** 这是stripe cache连续命中效应的结果——顺序写使cache中的数据不断累积，让RCW的实际读IO次数远低于理论值。
3. **4K随机写场景下，rmw_level影响很小**（±18%），因为瓶颈在磁盘寻道而非策略选择。
4. **高队列深度（≥128）下，FSW接管后rmw_level影响可以忽略。** 优化重点应放在触发FSW上（参见3.5节）。
5. **大块IO场景下，保持默认rmw_level=1最优。** rmw=0强制RCW会多读不必要的盘。
6. **避免使用rmw_level=2。** 在RAID6顺序写低队列下可能造成50%的性能下降。
7. **RAID6不建议盲目设为rmw_level=0。** 实测未发现"双校验RMW计算代价过大"导致的普遍性能问题——内核的动态代价计算已经考虑了双校验开销，能自动做出合理选择。
...
> **建议6：应用层的IO提交方式直接决定RAID5能否充分利用plug机制。**
>
...
那单盘3.5倍提升从哪来？看延迟数据就清楚了：d=1时单个IO的`clat`仅52μs，d=128时增长到2,172μs（增长了42倍），但因为队列中同时有128个IO在飞行，吞吐量仍然在涨（`IOPS ≈ iodepth / clat`）。这是典型的**设备级并行处理**——virtio-blk后端可以同时处理多个请求，虽然每个请求的延迟变大了，但总吞吐量随并发数增加而提升。

> **技术要点**：在本测试环境下，底层设备是通过virtio-blk接口暴露的SATA硬盘，write_cache设置为write back，且mq-deadline调度器在4K粒度的顺序IO下不做合并（因为这些IO已经是最小粒度了）。性能的提升纯粹来自设备后端的并发处理能力，不涉及块层IO合并。
...
1. **SATA盘（virtio-blk接口）的设备级并行化是有选择性的**——顺序写能从并发提交中受益（3.5倍提升），但随机写不能。这与SATA盘的物理特性有关：顺序写请求在磁盘内部可以被更高效地批量处理（磁头不需要频繁寻道）；而随机写地址分散，磁盘需要频繁寻道，后端无法做类似优化。
...
这导致平均值出现了**d=4（35,914）> d=8（33,095）> d=16（32,967）**的反常现象。这种波动在SATA机械硬盘上并不罕见——受磁盘内部队列管理、NCQ调度策略以及宿主机虚拟化层的影响，中等iodepth区间的性能抖动较大。相比之下RAID5各轮数据非常稳定（CV通常<5%），这是因为RAID5的多盘并行天然有平滑效果。但这种波动不影响整体结论——在低iodepth（d≤4）和高iodepth（d≥64）区间，数据都很一致且趋势清晰。
...
**关键差异在于顺序读场景**：Symmetric（对称）布局中，跨stripe的连续数据块（如D2→D3）分布在不同磁盘上，顺序读可以充分利用所有磁盘的并行带宽。而Asymmetric（不对称）布局中，跨stripe时数据块回到Disk0重新开始，导致某些磁盘在stripe交界处空闲一拍，略微降低了并行度。而Parity-0和Parity-N将校验盘固定在一块盘上，该盘的写负载是其他盘的数倍，成为整个阵列的写性能瓶颈。

Linux默认使用`left-symmetric`（左对称）布局，这种布局下每个stripe的数据块在多个成员盘上的分布更均匀，顺序读时可以最大化磁盘并发度。

> **建议7：保持默认的`left-symmetric`算法。实测表明它在顺序写低队列场景下有约20%~25%的优势，其他场景下与其余轮转布局持平，是最稳健的选择。避免使用parity-0/parity-N——随机IO下比轮转布局差15%~23%。**

#### 实验验证：布局算法对RAID5性能的实际影响

上面的分析是从数据分布的角度给出的理论推导，那么实际测试中各布局的性能差异有多大？是几个百分点还是几倍？哪些场景差异显著？我们用6种布局的完整对照实验来回答。

**测试方法**：在同一组5×10.9TB SATA HDD上，依次创建6种布局的RAID5（chunk=256K，`--assume-clean`），每种布局测试10种负载（顺序读×4 + 顺序写×4 + 随机读写×2），每种负载20s × 3轮取平均，测完销毁再创建下一种布局。测试环境与3.3.6节的rmw_level实验完全一致。

**顺序读（IOPS，3轮平均）**

| 负载 | la | ra | ls（默认） | rs | parity-0 | parity-N |
|------|---:|---:|---:|---:|---:|---:|
| seqread 4k d=1 | 14,131 | 12,481 | 12,634 | 12,785 | 13,553 | 14,061 |
| seqread 4k d=32 | 26,205 | 32,774 | 28,562 | 30,289 | 32,902 | 35,975 |
| seqread 4k d=128 | 32,541 | 35,060 | 27,049 | 28,109 | 32,870 | 44,051 |
| seqread 1M d=16 | 1,043 | 1,042 | 1,042 | 1,043 | 1,040 | 1,064 |

**顺序写（IOPS，3轮平均）**

| 负载 | la | ra | ls（默认） | rs | parity-0 | parity-N |
|------|---:|---:|---:|---:|---:|---:|
| seqwrite 4k d=1 | 1,464 | 1,429 | 1,723 | 1,777 | 1,414 | 1,407 |
| seqwrite 4k d=32 | 12,055 | 11,171 | 9,226 | 8,857 | 9,083 | 8,392 |
| seqwrite 4k d=128 | 17,746 | 19,421 | 17,635 | 20,400 | 20,230 | 18,992 |
| seqwrite 1M d=16 | 770 | 793 | 797 | 774 | 782 | 776 |

**随机IO（IOPS，3轮平均）**

| 负载 | la | ra | ls（默认） | rs | parity-0 | parity-N |
|------|---:|---:|---:|---:|---:|---:|
| randread 4k d=128 j=4 | 1,084 | 1,085 | 1,078 | 1,075 | 879 | 879 |
| randwrite 4k d=128 j=4 | 100 | 102 | 103 | 100 | 87 | 88 |

从这三组数据中可以看到几个清晰的模式：

**1. parity-0/N的随机IO比轮转布局差15%~23%——校验盘瓶颈被实测验证。**

随机读中，la/ra/ls/rs四种轮转布局都在1,075~1,085 IOPS，而parity-0和parity-N只有879——低了约**19%**。随机写也有类似差距（100 vs 87，差**15%**）。这直接验证了上面布局分析中"Parity-0/N的写负载均衡★☆☆☆☆"的评价。

原因很直观：parity-0/N的校验盘固定在一块盘上。对于随机写，每次写操作都要更新校验，该盘的写负载是其他数据盘的N倍。即使是随机读（不涉及校验写入），校验盘固定也意味着只有3块盘存数据（第4块全是校验），有效的数据并行度从4盘降到了3盘。而轮转布局下校验块均匀分布在所有盘上，不存在这个瓶颈。

**2. 四种轮转布局（la/ra/ls/rs）之间，大多数场景差异在±10%以内。**

除了顺序写的个别负载，la/ra/ls/rs的性能非常接近。以数据最稳定的两组为例：
- 随机读（L9）：1,075 ~ 1,085，极差仅**0.9%**
- 1M顺序读（L4）：1,042 ~ 1,043，极差仅**0.1%**

这说明在SATA HDD + virtio-blk环境下，symmetric和asymmetric布局之间的理论差异（跨stripe数据块是否分布在不同盘上）**被磁盘本身的毫秒级延迟和虚拟化层的调度所掩盖**。布局的纳秒级寻址差异相对于磁盘的物理寻道时间微不足道。

**3. 4K顺序写低队列（d=1）下，symmetric布局有约20%~25%的优势。**

这是整组实验中最有意义的差异：ls=1,723、rs=1,777，而la=1,464、ra=1,429、parity-0=1,414、parity-N=1,407。Symmetric布局比asymmetric和parity布局快了约20%~26%。

这个优势的原理与3.3.6节rmw_level实验中发现的stripe cache效应一脉相承。Symmetric布局中，跨stripe的连续数据块（如stripe 0的D2和stripe 1的D3）分布在**不同**磁盘上。当应用顺序写这些块时，它们属于不同stripe，但因为分布在不同盘上，一个stripe中已写入的数据更容易留在stripe cache中供下一个stripe使用——RCW路径的实际读IO次数更低。而asymmetric布局中跨stripe的数据块回到Disk0重新开始，cache命中的连续性被打断。

不过需要注意的是，4K顺序读（L1~L3）的数据波动非常大。以L3（d=128）为例，ls的3轮数据为23,425 / 23,644 / 34,079（最大差45%）。这种波动在SATA机械盘上是常见现象（3.5节的plug/unplug实验中也有类似观察），由磁盘内部队列管理和宿主机虚拟化层调度共同导致。因此4K顺序读的布局对比结论不具备统计显著性，仅供参考。相比之下，1M顺序读/写和随机IO的数据非常稳定（CV<5%），结论可靠。

**4. 1M大块IO完全不受布局影响。**

1M顺序读的6种布局都稳定在1,040~1,064 IOPS（极差仅2%），1M顺序写也只有770~797（极差4%）。这与预期一致——1M远大于256K的chunk，每次IO都跨越多个chunk和stripe，布局对单个chunk内数据分布的差异在大块IO的尺度下被完全抹平。

**实验结论：**

1. **parity-0和parity-N不应使用**——随机IO下比轮转布局低15%~23%，校验盘固定导致的负载不均衡在实测中得到了明确验证。
2. **四种轮转布局（la/ra/ls/rs）之间差异很小**，大多数场景不超过10%，没有一种在所有场景下一致胜出。
3. **Symmetric布局在4K顺序写低队列下有约20%~25%的优势**，这是唯一一个有实际意义的差异，来源于跨stripe数据分布对stripe cache连续命中的促进效果。
4. **大块IO（1M）完全不受布局影响**，六种布局性能一致。
5. **保持默认的`left-symmetric`是最稳健的选择**——它在顺序写低队列有优势，其他场景不输。没有实测数据支持更换为其他布局。
...
4. **RAID5写性能的核心**是尽量触发Full Stripe Write，避免RMW：
   ...
   - **rmw_level参数的影响取决于IO模式**（详见3.3.6节实验验证）：默认的`rmw_level=1`在大多数场景下最优，让内核动态选择RMW/RCW。但4K顺序写低队列场景下，`rmw_level=0`（禁用RMW）可带来最高71%的提升——这是stripe cache连续命中效应的结果，顺序写使cache中数据不断累积，让RCW的实际读IO次数远低于理论值。避免使用`rmw_level=2`（RAID6顺序写下可能下降50%）。RAID6不建议盲目设为`rmw_level=0`——内核的动态代价计算已能处理好双校验开销
